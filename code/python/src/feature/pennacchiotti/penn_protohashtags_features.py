...
def match_extracted_dictionary(dictionaries: dict, csv_input_feature_file, col_id, outfile,
    ...
    for row in df:
        ...
        hashtags = re.findall(r'\B#\w*[a-zA-Z]+\w*', target_text)

        if len(hashtags) > 1:
            target_text=" ".join(hashtags).lower().strip()
        ...
        for k in dict_labels:
            ...
            matchsum, total_words, match_freq = \
                count_word_matches(dictionary, target_text)
            if total_words==0:
                total_words=1
...
def count_word_matches(dictionary, target_text):
    norm_toks = target_text.split(" ")
...
if __name__ == "__main__":
    ...
    dictionary_folder = "/home/zz/Work/msm4phi_data/paper2/reported/penn/dictionary_hashtag"
    ...
    match_extracted_dictionary(extracted_dictionaries, file,
                               col_id,
                               outfolder + "/feature_protohashtags.csv",
                               target_text_cols)
